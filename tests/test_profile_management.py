...
def test_get_user_details(create_client_and_cleanup, fixture_create_user):
    ...
    response = create_client_and_cleanup.patch("/update_profile_details/v0", json=update_payload, headers=headers)
    assert response.status_code == 200
    ...
    assert data["username"] == create_user_output["data"]["main"]["username"]
