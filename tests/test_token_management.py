...
def test_generate_access_token(create_client_and_cleanup, fixture_create_user):
    ...
    response = create_client_and_cleanup.get("/generate_access_token/v0", headers=headers)
    assert response.status_code == 200
    assert response.json()["message"] == messages["TOKEN_GENERATION_SUCCESSFUL"]
...
def test_logout_specific_app(create_client_and_cleanup, fixture_create_user):
    ...
    response = create_client_and_cleanup.delete("/logout/apps/v0", json=payload, headers=headers)
    assert response.status_code == 200
...
def test_logout_all_sessions(create_client_and_cleanup, fixture_create_user):
    ...
    # Try to use the same access token (should fail)
    response = create_client_and_cleanup.get("/get_user_details/v0", headers=headers)
    assert response.status_code == 400
    assert response.json()["message"] == messages["INCORRECT_ACCESS_TOKEN"]
